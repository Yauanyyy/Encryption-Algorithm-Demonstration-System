window.algorithms.aes = (() => {
    // AES Constants including Inverse S-Box
    const C = {
        S_BOX: [],
        INV_S_BOX: [],
        RCON: [0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36]
    };
    
    const formatState = (state, title) => {
        let html = `<h4>${title}</h4><div class="state-matrix aes">`;
        for (let c = 0; c < 4; c++) { 
            for (let r = 0; r < 4; r++) {
                html += `<div class="state-cell">${state[r][c].toString(16).padStart(2, '0').toUpperCase()}</div>`;
            }
        }
        return html + '</div>';
    };
    
    function keyExpansion(key) {
        const Nk = 8; // words in key for AES-256
        const Nr = 14; // rounds for AES-256
        const w = [];
        for (let i = 0; i < Nk; i++) {
            w[i] = [key[4 * i], key[4 * i + 1], key[4 * i + 2], key[4 * i + 3]];
        }

        for (let i = Nk; i < 4 * (Nr + 1); i++) {
            let temp = [...w[i - 1]];
            if (i % Nk === 0) {
                temp = [temp[1], temp[2], temp[3], temp[0]]; // RotWord
                temp = temp.map(b => C.S_BOX[b]); // SubWord
                temp[0] ^= C.RCON[i / Nk];
            } else if (Nk > 6 && i % Nk === 4) { // for AES-256
                temp = temp.map(b => C.S_BOX[b]); // SubWord
            }
            w[i] = w[i - Nk].map((b, j) => b ^ temp[j]);
        }
        return w;
    }

    const subBytes = (state) => state.map(row => row.map(b => C.S_BOX[b]));
    const invSubBytes = (state) => state.map(row => row.map(b => C.INV_S_BOX[b]));

    const shiftRows = (state) => {
        let s = state.map(row => [...row]);
        s[1] = [s[1][1], s[1][2], s[1][3], s[1][0]];
        s[2] = [s[2][2], s[2][3], s[2][0], s[2][1]];
        s[3] = [s[3][3], s[3][0], s[3][1], s[3][2]];
        return s;
    };
    const invShiftRows = (state) => {
        let s = state.map(row => [...row]);
        s[1] = [s[1][3], s[1][0], s[1][1], s[1][2]];
        s[2] = [s[2][2], s[2][3], s[2][0], s[2][1]];
        s[3] = [s[3][1], s[3][2], s[3][3], s[3][0]];
        return s;
    };
    
    const gmul = (a, b) => {
        let p = 0;
        for (let i = 0; i < 8; i++) {
            if (b & 1) p ^= a;
            let hi_bit_set = a & 0x80;
            a <<= 1;
            if (hi_bit_set) a ^= 0x11b;
            b >>= 1;
        }
        return p & 0xff;
    };
    
    const mixColumns = (state) => {
        let s = state.map(row => [...row]);
        for (let c = 0; c < 4; c++) {
            let a = [s[0][c], s[1][c], s[2][c], s[3][c]];
            s[0][c] = gmul(a[0], 2) ^ gmul(a[1], 3) ^ a[2] ^ a[3];
            s[1][c] = a[0] ^ gmul(a[1], 2) ^ gmul(a[2], 3) ^ a[3];
            s[2][c] = a[0] ^ a[1] ^ gmul(a[2], 2) ^ gmul(a[3], 3);
            s[3][c] = gmul(a[0], 3) ^ a[1] ^ a[2] ^ gmul(a[3], 2);
        }
        return s;
    };
    
    const invMixColumns = (state) => {
        let s = state.map(row => [...row]);
        for (let c = 0; c < 4; c++) {
            let a = [s[0][c], s[1][c], s[2][c], s[3][c]];
            s[0][c] = gmul(a[0], 0x0e) ^ gmul(a[1], 0x0b) ^ gmul(a[2], 0x0d) ^ gmul(a[3], 0x09);
            s[1][c] = gmul(a[0], 0x09) ^ gmul(a[1], 0x0e) ^ gmul(a[2], 0x0b) ^ gmul(a[3], 0x0d);
            s[2][c] = gmul(a[0], 0x0d) ^ gmul(a[1], 0x09) ^ gmul(a[2], 0x0e) ^ gmul(a[3], 0x0b);
            s[3][c] = gmul(a[0], 0x0b) ^ gmul(a[1], 0x0d) ^ gmul(a[2], 0x09) ^ gmul(a[3], 0x0e);
        }
        return s;
    };

    const addRoundKey = (state, w, round) => {
        let s = state.map(row => [...row]);
        for (let r = 0; r < 4; r++) {
            for (let c = 0; c < 4; c++) {
                s[r][c] ^= w[round * 4 + c][r];
            }
        }
        return s;
    };
    
    function encrypt(block, key) {
        const Nr = 14;
        const w = keyExpansion(key);
        let state = []; 
        for(let r=0; r<4; r++) {
            state[r] = [block[r], block[r+4], block[r+8], block[r+12]];
        }
        
        let steps = formatState(state, 'Initial State');
        state = addRoundKey(state, w, 0);
        steps += formatState(state, 'After AddRoundKey (Round 0)');

        for (let round = 1; round < Nr; round++) {
            state = subBytes(state);
            steps += formatState(state, `Round ${round}: After SubBytes`);
            state = shiftRows(state);
            steps += formatState(state, `Round ${round}: After ShiftRows`);
            state = mixColumns(state);
            steps += formatState(state, `Round ${round}: After MixColumns`);
            state = addRoundKey(state, w, round);
            steps += formatState(state, `Round ${round}: After AddRoundKey`);
        }

        state = subBytes(state);
        steps += formatState(state, `Round ${Nr}: After SubBytes`);
        state = shiftRows(state);
        steps += formatState(state, `Round ${Nr}: After ShiftRows`);
        state = addRoundKey(state, w, Nr);
        steps += formatState(state, 'Final State (After AddRoundKey)');

        let output = new Array(16);
        for(let r=0; r<4; r++) {
            for(let c=0; c<4; c++) output[c*4+r] = state[r][c];
        }
        return { result: output, steps };
    }
    
    function decrypt(block, key) {
        const Nr = 14;
        const w = keyExpansion(key);
        let state = [];
        for (let r = 0; r < 4; r++) {
            state[r] = [block[r], block[r + 4], block[r + 8], block[r + 12]];
        }

        let steps = formatState(state, 'Initial Ciphertext State');
        state = addRoundKey(state, w, Nr);
        steps += formatState(state, 'After AddRoundKey (Round 14)');

        for (let round = Nr - 1; round >= 1; round--) {
            state = invShiftRows(state);
            steps += formatState(state, `Round ${round}: After InvShiftRows`);
            state = invSubBytes(state);
            steps += formatState(state, `Round ${round}: After InvSubBytes`);
            state = addRoundKey(state, w, round);
            steps += formatState(state, `Round ${round}: After AddRoundKey`);
            state = invMixColumns(state);
            steps += formatState(state, `Round ${round}: After InvMixColumns`);
        }

        state = invShiftRows(state);
        steps += formatState(state, 'Round 0: After InvShiftRows');
        state = invSubBytes(state);
        steps += formatState(state, 'Round 0: After InvSubBytes');
        state = addRoundKey(state, w, 0);
        steps += formatState(state, 'Final State (Plaintext)');

        let output = new Array(16);
        for (let r = 0; r < 4; r++) {
            for (let c = 0; c < 4; c++) output[c * 4 + r] = state[r][c];
        }
        return { result: output, steps };
    }

    return {
        getHTML: () => `
            <div class="content-section">
                <h2>AES-256 (Advanced Encryption Standard)</h2>
                
                <div class="algo-brief">
                    <h3>算法简介</h3>
                    <p>高级加密标准（AES）是目前全球广泛使用的对称加密标准，旨在替代 DES。它采用置换-组合网络（SPN）结构，支持 128、192 和 256 位密钥，具有极高的安全性和效率。AES 现已成为政府、金融机构和各类软件中保护敏感数据的首选算法。</p>
                </div>

                <h3>加解密操作台</h3>
                <div class="operation-panel">
                    
                    <div class="form-group">
                        <label for="aes-input">数据块 (32个十六进制字符, 128位)</label>
                        <input type="text" id="aes-input" maxlength="32" placeholder="e.g., 00112233445566778899AABBCCDDEEFF">
                    </div>
                    <div class="form-group">
                        <label for="aes-key">密钥 (64个十六进制字符, 256位)</label>
                        <input type="text" id="aes-key" maxlength="64" placeholder="e.g., 000102030405060708090A0B0C0D0E0F101112131415161718191A1B1C1D1E1F">
                    </div>
                    <div class="form-group">
                        <label for="aes-mode">模式</label>
                        <select id="aes-mode">
                            <option value="encrypt">加密</option>
                            <option value="decrypt">解密</option>
                        </select>
                    </div>
                    <button id="aes-execute" class="btn btn-primary" style="margin-bottom: 1.5rem;">执行</button>
                    
                    <div class="form-group">
                        <label id="aes-output-label">结果 (十六进制)</label>
                        <div class="textarea-wrapper">
                            <textarea id="aes-output" readonly></textarea>
                             <button class="copy-btn" title="复制结果" onclick="Utils.copyToClipboard(document.getElementById('aes-output').value)">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16"><path d="M4 1.5H3a2 2 0 0 0-2 2V14a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V3.5a2 2 0 0 0-2-2h-1v1h1a1 1 0 0 1 1 1V14a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1V3.5a1 1 0 0 1 1-1h1v-1z"/><path d="M9.5 1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1-.5-.5v-1a.5.5 0 0 1 .5-.5h3z"/></svg>
                            </button>
                        </div>
                    </div>
                </div>

                <h3>过程可视化</h3>
                <div class="visualization-area" id="aes-steps" style="display: flex; flex-wrap: wrap; gap: 20px; justify-content: center;"></div>

                <h3>算法详细介绍</h3>
                <div id="algo-details-md" class="algo-details"></div>
            </div>
        `,
        init: async () => {
            const detailsContainer = document.getElementById('algo-details-md');
            if (detailsContainer && window.ALGO_CONTENT) {
                detailsContainer.innerHTML = window.ALGO_CONTENT.aes || '内容加载失败';
            }

            document.getElementById('aes-mode').addEventListener('change', (e) => {
                const isEncrypt = e.target.value === 'encrypt';
                // 移除此处修改 outputLabel 的代码，使其保持默认的"结果 (十六进制)"
                document.getElementById('aes-input').placeholder = isEncrypt ? '输入明文的十六进制' : '输入密文的十六进制';
            });
            
            document.getElementById('aes-execute').addEventListener('click', () => {
                const mode = document.getElementById('aes-mode').value;
                const hexInput = document.getElementById('aes-input').value.replace(/\s/g, '');
                const hexKey = document.getElementById('aes-key').value.replace(/\s/g, '');
                const hex128Regex = /^[0-9a-fA-F]{32}$/;
                const hex256Regex = /^[0-9a-fA-F]{64}$/;

                if (!hex128Regex.test(hexInput)) {
                    Utils.showAlert('错误：数据块必须是32个有效的十六进制字符。');
                    return;
                }
                if (!hex256Regex.test(hexKey)) {
                    Utils.showAlert('错误：AES-256密钥必须是64个有效的十六进制字符。');
                    return;
                }
                
                const hexToBytes = (hex) => {
                    const bytes = [];
                    for (let i = 0; i < hex.length; i += 2) {
                        bytes.push(parseInt(hex.substr(i, 2), 16));
                    }
                    return bytes;
                };

                const textBytes = hexToBytes(hexInput);
                const keyBytes = hexToBytes(hexKey);
                
                let resultBytes, steps;
                if (mode === 'encrypt') {
                    ({ result: resultBytes, steps } = encrypt(textBytes, keyBytes));
                    document.getElementById('aes-output').value = resultBytes.map(b => b.toString(16).padStart(2, '0').toUpperCase()).join('');
                } else {
                    ({ result: resultBytes, steps } = decrypt(textBytes, keyBytes));
                    // 修改：不再尝试 TextDecoder 解码，直接输出 Hex 字符串
                    document.getElementById('aes-output').value = resultBytes.map(b => b.toString(16).padStart(2, '0').toUpperCase()).join('');
                }
                
                document.getElementById('aes-steps').innerHTML = steps;
            });
        }
    };
})();